...
|---|---|---|---|
| S1 | **Critical — FIXED in this PR** | Next.js version carried an RCE advisory (React flight protocol, GHSA-9qr9-h5gf-34mp) plus Server-Actions source-exposure and DoS advisories. Vercel independently enforced this mid-review: it rejected this branch's preview deployment with "Vulnerable version of Next.js detected" — i.e., **no deploy of punktual.co could go live on the old version**. Upgraded to 15.5.19 in this PR (commit `1e271cd`); critical advisory cleared, build verified. | `package.json` (was `next ^15.3.2`) |
