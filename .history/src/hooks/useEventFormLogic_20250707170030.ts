//src/hooks/useEventFormLogic.ts
...
export const useEventFormLogic = (): EventFormLogicReturn => {
  ...
  // REMOVED: The competing useEffect that was setting defaults
  // EventContext already provides good defaults, no need to override them
  ...
  // FIXED: Batched field change handler - single updateEvent call
  const handleFieldChange = (field: keyof EventData, value: string | boolean | number | string[] | undefined): void => {
    // Batch all related updates into ONE call to prevent multiple state updates
    ...
    if (field === 'startDate' && typeof value === 'string') {
      if (!eventData.endDate || new Date(value) > new Date(eventData.endDate)) {
        updates.endDate = value; // Add to same batch
      }
    }
    ...
    if (field === 'startTime' && typeof value === 'string' && !eventData.isAllDay) {
      if (!eventData.endTime || (eventData.startDate === eventData.endDate && value >= eventData.endTime)) {
        updates.endTime = addHoursToTime(value, 1); // Add to same batch
      }
    }
    ...
    if (field === 'isAllDay') {
      if (value) {
        ...
        updates.endTime = addHoursToTime(startTime, 1);
      }
    }
    
    // SINGLE updateEvent call with all changes batched together
    updateEvent(updates);
  };
