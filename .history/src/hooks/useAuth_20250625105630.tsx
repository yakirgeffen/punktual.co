...
import type { Subscription, AuthChangeEvent, AuthError } from '@supabase/supabase-js';
import type { 
  SupabaseUser, 
  SupabaseSession, 
  AuthContextType, 
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  // Initialize with getSession timeout to prevent hanging
  const initializeAuth = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const timeoutPromise = new Promise<{ data: { session: SupabaseSession | null }, error: AuthError | null }>((resolve) => 
        setTimeout(() => resolve({ 
          data: { session: null }, 
          error: { message: 'getSession timeout', name: 'TimeoutError' } as AuthError 
        }), 3000)
      );
