 ...
 */
export const cityTimezoneMap: Record<string, string> = {
 ...
 */
export const searchTimezonesAndCities = (searchTerm: string, allTimezones: TimezoneData[]): TimezoneData[] => {
  if (!searchTerm.trim()) {
  ...
  const directMatches = allTimezones.filter(tz => 
    tz.searchTerms && tz.searchTerms.includes(searchLower) && 
  ...
  if (results.length === 0) {
    const partialMatches = allTimezones.filter(tz => 
      (tz.city && tz.city.includes(searchLower)) || 
      (tz.region && tz.region.includes(searchLower)) ||
      (tz.label && tz.label.toLowerCase().includes(searchLower))
    );
 ...
 */
export const formatTimezoneDisplay = (timezone: TimezoneData): string => {
  if (!timezone) return '';
