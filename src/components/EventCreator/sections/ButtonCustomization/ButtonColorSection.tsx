...
const colorNames = Object.keys(tailwindColors) as Array<keyof typeof tailwindColors>;
 ...
 return (
   <>
     <div className="flex items-center gap-3">
       ...
       >
         <div 
           className="w-3 h-3 rounded-small border" 
                 ...
                 {Array.from({ length: 11 }, (_, rowIndex) => (
                   <tr key={rowIndex}>
                     {colorNames.map((colorName) => {
                       const color = tailwindColors[colorName][rowIndex];
                       const isSelected = currentColor === color;
                       return (
                         <td key={colorName} className="p-0">
                           <button
                             onClick={() => selectColor(color)}
                             className={`w-6 h-6 border hover:scale-110 transition-transform relative ${
